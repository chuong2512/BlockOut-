...
public class SaferioToggle : MonoBehaviour
{
	...
	private void Awake() { switchButton.onClick.AddListener(Switch); }

	public void RegisterOnSwitchedEvent(Action<bool> action) { onSwitched=action; }

	public void SetState(bool isOn)
	{
		activeBackground.gameObject.SetActive(isOn);
	...
	public void Switch()
	{
		...
		activeBackground.gameObject.SetActive(_isOn);
		...
		/*Tween.LocalPositionX(icon,iconLocalPositionX,duration: transitionDuration)
			.OnComplete(() => { onSwitched?.Invoke(_isOn); });*/
	}
}
