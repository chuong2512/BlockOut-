...
public class WinPopup : BasePopup
{
	...
	[Header("CUSTOMIZE")] [SerializeField] private float timeCollectCoin;
	...
	private void CollectCoin(int numCoin)
	{
		...
		_tweens.Add(
		Tween.Custom(0,numCoin,duration: timeCollectCoin,onValueChange: newVal => { coinText.text=$"{(int) newVal}"; })
			.OnComplete(() =>
			{
	...
	{
		_tweens.Add(Tween.Delay(1).OnComplete(() => { HideImmediately(); }));

		goLevelEvent?.Invoke(currentLevel.Value);
	}

	private void ReturnHome() { Addressables.LoadSceneAsync(GameConstants.MENU_SCENE); }
